...
| Web tests must run the `--wasm` lane; the `--platform chrome` DDC lane hangs at suite load on 3.44.4 (fixed on master by the web-test loader rewrite, so re-check when the SDK pin moves — the `--wasm` requirement may relax) | flutter-tools `flutter_web_platform.dart`; verified fixed on master 3.46 pre | `make test-web` runs `--wasm` |
